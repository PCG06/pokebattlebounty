...
DOUBLE_BATTLE_TEST("Electro Boost activates in speed order in a double battle")
{
    ...
    GIVEN {
        PLAYER(SPECIES_PIKACHU) { Speed(4); }
        PLAYER(SPECIES_RAICHU) { Speed(6); }
        OPPONENT(SPECIES_WOBBUFFET) { Speed(5); }
        OPPONENT(SPECIES_MINUN) { Speed(3); }
    } WHEN {
        TURN {}
    } SCENE {
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_CHARGED_UP, playerRight);
        // opponentLeft is not Electric-type, does not activate
        ANIMATION(ANIM_TYPE_GENERAL, B_ANIM_CHARGED_UP, playerLeft);
