...
static bool32 SetupGraphics(void);
static bool32 LoadGraphics(void);
static void QuestMenu_InitWindows(void);
static bool32 InitBackgrounds(void);
static void InitItems(void);
static bool32 AllocateResourcesForListMenu(void);
...
static bool32 IfScrollIsOutOfBounds(void);
static bool32 IfRowIsOutOfBounds(void);
static void SaveScrollAndRow(s16 *data);

static void ClearModeOnStartup(void);
static u32 ManageMode(u8 action);
static u32 ToggleAlphaMode(u8 mode);
static u32 ToggleSubquestMode(u8 mode);
static u32 IncrementMode(u8 mode);
static bool32 IsSubquestMode(void);
static bool32 IsNotFilteredMode(void);
static bool32 IsAlphaMode(void);

static void BuildMenuTemplate(void);
static u32 GetModeAndGenerateList();
static u32 CountNumberListRows();
static u8 *DefineQuestOrder();
static u32 GenerateSubquestList();
static u32 GenerateList(bool32 isFiltered);
...
static u32 CountCompletedQuests(void);
...
static bool32 DoesQuestHaveChildrenAndNotInactive(u16 itemId);
...
static bool32 UNUSED IsQuestUnlocked(s32 questId);
static bool32 IsQuestActiveState(s32 questId);
static bool32 IsQuestInactiveState(s32 questId);
static bool32 IsQuestRewardState(s32 questId);
static bool32 IsQuestCompletedState(s32 questId);
static bool32 IsSubquestCompletedState(s32 questId);
...
static u32 GenerateSubquestState(u8 questId);
static u32 GenerateQuestState(u8 questId);
...
static bool32 CheckSelectedIsCancel(u8 selectedQuestId);
...
static bool32 HandleFadeOut(u8 taskId);
static void PrepareFadeIn(u8 taskId);
static bool32 HandleFadeIn(u8 taskId);
...
static void PlaceTopMenuScrollIndicatorArrows(void)
{
    ...
    sStateDataPtr->scrollIndicatorArrowPairId =
          AddScrollIndicatorArrowPairParameterized(2, 94, 8, 90, (listSize - sStateDataPtr->maxShowed), 110, 110, &sListMenuState.scroll);
}
...
static u32 CountNumberListRows()
{
    ...
    if (IsSubquestMode())
    {
        return gMainQuests[sStateDataPtr->parentQuest].numSubquests + 1;
    }
...
static u8 *DefineQuestOrder()
{
    ...
    if (IsAlphaMode())
    {
        for (c = 0; c < QUEST_COUNT; c++)
        {
            ...
            {
                if (StringCompare(gMainQuests[sortedList[c]].name, gMainQuests[sortedList[d]].name) > 0)
                {
...
static u32 GenerateSubquestList()
{
    ...
    for (numRow = 0; numRow < gMainQuests[parentQuest].numSubquests; numRow++)
    {
...
{
    u8 uniqueId = gMainQuests[quest].subquests[childQuest].id;
    ...
    case FLAG_GET_COMPLETED:
        return gSaveBlock2Ptr->subQuestData[index] & mask;
    case FLAG_SET_COMPLETED:
        gSaveBlock2Ptr->subQuestData[index] |= mask;
        return 1;
    }
...
u8 QuestMenu_GetSetQuestState(u8 quest, u8 caseId)
{
    ...
    */
    ...
    {
    case FLAG_GET_UNLOCKED:
        return gSaveBlock2Ptr->mainQuestData[index] & mask;
    case FLAG_SET_UNLOCKED:
        gSaveBlock2Ptr->mainQuestData[index] |= mask;
        return 1;
    case FLAG_GET_INACTIVE:
        bit2 = bit + 1;
        bit3 = bit + 2;
        ...
        mask3 = 1 << bit3;
        return !(gSaveBlock2Ptr->mainQuestData[index] & mask) && \
               !(gSaveBlock2Ptr->mainQuestData[index2] & mask2) && \
               !(gSaveBlock2Ptr->mainQuestData[index3] & mask3);
    case FLAG_GET_ACTIVE:
        return gSaveBlock2Ptr->mainQuestData[index] & mask;
    case FLAG_SET_ACTIVE:
        gSaveBlock2Ptr->mainQuestData[index] |= mask;
...
static u32 CountCompletedQuests(void)
{
    ...
    if (IsSubquestMode())
    {
        for (i = 0; i < gMainQuests[parentQuest].numSubquests; i++)
        {
...
static void PopulateEmptyRow(u8 countQuest)
{
    sQuestNamePointer = StringCopy(sQuestNameArray[countQuest], sText_Empty);
}

static void PrependQuestNumber(u8 countQuest)
{
    sQuestNamePointer = ConvertIntToDecimalStringN(sQuestNameArray[countQuest], countQuest + 1, STR_CONV_MODE_LEFT_ALIGN, 2);
    sQuestNamePointer = StringAppend(sQuestNamePointer, sText_DotSpace);
}

static void SetFavoriteQuest(u8 countQuest)
{
    sQuestNamePointer = StringAppend(sQuestNameArray[countQuest], sText_ColorGreen);
}
...
static void PopulateQuestName(u8 countQuest)
{
    ...
    {
        sQuestNamePointer = StringAppend(sQuestNameArray[countQuest], gMainQuests[countQuest].name);
        AddSubQuestButton(countQuest);
    }
    else
    {
        StringAppend(sQuestNameArray[countQuest], sText_Unk);
    }
}
...
static void PopulateSubquestName(u8 parentQuest, u8 countQuest)
{
    ...
    {
        sQuestNamePointer = StringAppend(sQuestNamePointer, gMainQuests[parentQuest].subquests[countQuest].name);
    }
    else
    {
...
static void PopulateListRowNameAndId(u8 row, u8 countQuest)
{
    sListMenuItems[row].name = sQuestNameArray[countQuest];
    sListMenuItems[row].id = countQuest;
}

static bool32 DoesQuestHaveChildrenAndNotInactive(u16 itemId)
{
    if (gMainQuests[itemId].numSubquests != 0
...
static void AddSubQuestButton(u8 countQuest)
{
    ...
    {
        sQuestNamePointer = StringAppend(sQuestNameArray[countQuest], sText_SubQuestButton);
    }
...
static void GenerateQuestLocation(s32 questId)
{
    if (!IsSubquestMode())
    {
        StringCopy(gStringVar2, gMainQuests[questId].map);
    }
    else
    {
        StringCopy(gStringVar2, gMainQuests[sStateDataPtr->parentQuest].subquests[questId].map);
    }
...
static void GenerateQuestFlavorText(s32 questId)
{
    if (IsSubquestMode() == FALSE)
    {
        ...
        {
            StringCopy(gStringVar1, gMainQuests[questId].donedesc);
        }
    }
    else
    {
        ...
        {
            StringCopy(gStringVar1, gMainQuests[sStateDataPtr->parentQuest].subquests[questId].desc);
        }
        else
        {
...
static void UpdateQuestFlavorText(s32 questId)
{
    StringCopy(gStringVar1, gMainQuests[questId].desc);
}
...
bool32 IsQuestRewardState(s32 questId)
{
...
static void DetermineSpriteType(s32 questId)
{
    ...
    if (IsSubquestMode() == FALSE)
    {
        spriteId = gMainQuests[questId].sprite;
        spriteType = gMainQuests[questId].spritetype;
    ...
    {
        spriteId =
              gMainQuests[sStateDataPtr->parentQuest].subquests[questId].sprite;
        spriteType =
              gMainQuests[sStateDataPtr->parentQuest].subquests[questId].spritetype;
        QuestMenu_CreateSprite(spriteId, sStateDataPtr->spriteIconSlot, spriteType);
    }
    else
    {
...
static u32 GenerateSubquestState(u8 questId)
{
    ...
    {
        StringCopy(gStringVar4, gMainQuests[parentQuest].subquests[questId].type);
    }
    else
    {
...
static void GenerateNumeratorNumQuests(void)
{
    ...
    if (IsSubquestMode())
    {
        ConvertIntToDecimalStringN(gStringVar2, gMainQuests[parentQuest].numSubquests, STR_CONV_MODE_LEFT_ALIGN, 6);
        ConvertIntToDecimalStringN(gStringVar1, CountCompletedQuests(), STR_CONV_MODE_LEFT_ALIGN, 6);
    }
}
...
static void GenerateMenuContext(void)
{
    ...
    {
    case SORT_DEFAULT:
        sQuestNamePointer = StringCopy(sQuestNameArray[QUEST_ARRAY_COUNT], sText_AllHeader);
        break;
    case SORT_INACTIVE:
        sQuestNamePointer = StringCopy(sQuestNameArray[QUEST_ARRAY_COUNT], sText_InactiveHeader);
        break;
    case SORT_ACTIVE:
        sQuestNamePointer = StringCopy(sQuestNameArray[QUEST_ARRAY_COUNT], sText_ActiveHeader);
        break;
    case SORT_REWARD:
        sQuestNamePointer = StringCopy(sQuestNameArray[QUEST_ARRAY_COUNT], sText_RewardHeader);
        break;
    case SORT_DONE:
        sQuestNamePointer = StringCopy(sQuestNameArray[QUEST_ARRAY_COUNT], sText_CompletedHeader);
        break;
    }

    if (IsAlphaMode())
    {
        sQuestNamePointer = StringAppend(sQuestNameArray[QUEST_ARRAY_COUNT], sText_AZ);
    }
    if (IsSubquestMode())
    {
        sQuestNamePointer = StringCopy(sQuestNameArray[QUEST_ARRAY_COUNT], gMainQuests[parentQuest].name);
...
{
    StringCopy(dst, gMainQuests[questId].name);
}

void QuestMenu_CopySubquestName(u8 *dst, u8 parentId, u8 childId)
{
    StringCopy(dst, gMainQuests[parentId].subquests[childId].name);
}

void QuestMenu_ResetMenuSaveData(void)
{
    memset(&gSaveBlock2Ptr->mainQuestData, 0, sizeof(gSaveBlock2Ptr->mainQuestData));
    memset(&gSaveBlock2Ptr->subQuestData, 0, sizeof(gSaveBlock2Ptr->subQuestData));
}
